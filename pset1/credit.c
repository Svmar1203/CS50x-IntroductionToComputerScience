...
int main(void)
{
    ...
    do 
    {
        card = get_long("Number: ");//Prompt from user card number
    }
    while (card < 0);
    
    int card1, card2, card3, card4, card5, card6, card7, card8;//card1 - second acrd number from end
    card1 = ((card % 100) / 10) * 2;
    ...
    int sum1 = card1 + card2 + card3 + card4 + card5 + card6 + card7 + card8;//summ all second numbers
    
    int card9, card10, card11, card12, card13, card14, card15, card16;
    
    card9 = (card % 10);// first card number from end
    ...
    int sum2 = card9 + card10 + card11 + card12 + card13 + card14 + card15 + card16;// summ all first numbers 
    int sum3 = sum1 + sum2;// summ all numbers
    ...
    if ((sum3 % 10) != 0)// if sum not divide by 10
    {
        printf("%s\n", "Card is INVALID");// card is invalide
        return 0;
    }   
    while (card > 0)
    {
    ...
    if (visa == 4 && (length == 13 || length == 16))// first number of visa card
    {
        printf("%s\n", "VISA");
        return 0;
    }
    ...
    if (length == 15 && (amex == 34 || amex == 37))// first numbers of Amex card
    {
        printf("%s\n", "AMEX");
        return 0;
    }
    ...
    if (length == 16 && (master == 51 || master == 52 || master == 53 || master == 54 || master == 55))//first numbers of Mastercard
    {
        printf("%s\n", "MASTERCARD");
        return 0;
    }
